#ifndef __HCITRANS_H_
#define __HCITRANS_H_
...
#define HCITR_ERROR_UNABLE_TO_OPEN_TRANSPORT 	(-1) /* Denotes that the there was an error opening the transport layer */
#define HCITR_ERROR_READING_FROM_PORT        	(-2) /* Denotes that an error occurred reading from the transport layer port */
#define HCITR_ERROR_WRITING_TO_PORT          	(-3) /* Denotes that an error occurred writing to the transport layer port */
#define HCITR_ERROR_INVALID_PARAMETER        	(-4) /* Denotes that an error occurred caused by an invalid parameter being passed to a function */
#define HCITR_ERROR_SUSPEND_ABORTED          	(-5) /* Denotes that a suspend operation aborted, most commonly because data was received while waiting for the transmit buffer to empty */

#define HCITR_TRANSPORT_ID             			1
#define HCITR_INTERRUPT_PRIORITY       			1
#define HCITR_INPUT_BUFFER_SIZE        			1056
#define HCITR_OUTPUT_BUFFER_SIZE       			1056
#define HCITR_Clear_Reset()             		LL_GPIO_SetOutputPin(HCITR_GPIO_PORT_2, (1 << HCITR_P2_RESET_PIN))
#define HCITR_Set_Reset()               		LL_GPIO_ResetOutputPin(HCITR_GPIO_PORT_2, (1 << HCITR_P2_RESET_PIN))
#define HCITR_Enable_UART_Periph_Clock()  		HCITR_RCC_CLK_CMD_ENABLE_UART(HCITR_RCC_CLK_UART)
#define HCITR_Disable_UART_Periph_Clock() 		HCITR_RCC_CLK_CMD_DISABLE_UART(HCITR_RCC_CLK_UART)
#define HCITR_Disable_Interrupts()      		__set_PRIMASK(1)
#define HCITR_Enable_Interrupts()       		__set_PRIMASK(0)
#define HCITR_DEBUG_PRINT              			BTPS_OutputMessage
