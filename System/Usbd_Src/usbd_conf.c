...
/**
  ...
  * @param  Device handle (PCD_HandleTypeDef*)
  * @retval None
  ***************************************************************************************************************************************
  */
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
	...
	if(USB == pcdHandle->Instance)
	{
		...
		/** USB GPIO Configuration
    	  ...
    	  *	PA12 ------> USB_DP
    	  */
		GPIO_InitStruct.Pin = BSP_USB_DM_PIN | BSP_USB_DP_PIN;
		GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
		GPIO_InitStruct.Pull = GPIO_NOPULL;
		GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
		GPIO_InitStruct.Alternate = GPIO_AF10_USB_FS;
		HAL_GPIO_Init(BSP_USB_PORT, &GPIO_InitStruct);
...
void HAL_PCD_MspDeInit(PCD_HandleTypeDef* pcdHandle)
{
	if(USB == pcdHandle->Instance)
	{
		...
		/** USB GPIO Configuration
    	  ...
    	  * PA12 ------> USB_DP
    	  */
		HAL_GPIO_DeInit(BSP_USB_PORT, BSP_USB_DM_PIN | BSP_USB_DP_PIN);
...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
{
	...
	if ( hpcd->Init.speed != PCD_SPEED_FULL){BSP_Error_Handler();}
    /* Set Speed. */
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
	...
	if (HAL_PCD_Init(&hpcd_USB_FS) != HAL_OK){BSP_Error_Handler();}
...
uint8_t USBD_LL_IsStallEP(USBD_HandleTypeDef *pdev, uint8_t ep_addr)
{
	...
	if(0x80 == (ep_addr & 0x80)){return hpcd->IN_ep[ep_addr & 0x7F].is_stall;}
